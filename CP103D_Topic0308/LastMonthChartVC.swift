//
//  LastMonthChartVC.swift
//  CP103D_Topic0308
//
//  Created by 方錦泉 on 2019/4/5.
...
class LastMonthChartVC: UIViewController , ChartViewDelegate {
            ...
            if error == nil {
                if data != nil {
                    ...
                    if let result = try? decoder.decode([Order].self, from: data!) {
                        self.orders = result
                        self.ordersLastMonth = self.orders.filter { (order) -> Bool in
                            calendar.dateComponents(componentSet, from:order.date!).month == lastComponents.month
                        }
    ...
    func getAllOrderStatus() {
        ...
        var statusTwo = 0
        var statusThree = 0
        for i in 0...ordersLastMonth.count-1 {
            let orderStatus = ordersLastMonth[i].status
            if orderStatus == 0 {
                statusZero += 1
    ...
    func setup(chartView: PieChartView) {
        ...
        let centerText = NSMutableAttributedString(string: "上月訂單\n訂單完成率")
