//
//  ManagerManTableViewController.swift
//  CP103D_Topic0308
//
...
class ManagerManTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "managerManTableViewCell", for: indexPath) as! ManagerManTableViewCell
        cell.managerManCollectionviewOutlet.tag = indexPath.section
        cell.managerManCollectionviewOutlet.reloadData()
        return cell
    }
...
extension ManagerManTableViewController: UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        executeTask(url_server!, requestParam) { (data, response, error) in
            if error == nil {
                ...
                DispatchQueue.main.async { cell.managerManImageview.image = image }
            } else {
                print(error!.localizedDescription)
            }
        }
        
        cell.managerManLabel.text = goodsSubclass[indexPath.row].name
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "managerManSegue", sender: goodDetail)
    }
}
